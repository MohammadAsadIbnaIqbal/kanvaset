...
export const ProjectShareModal: React.FC<ProjectShareModalProps> = ({ projectId, onClose }) => {
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    try {
      await api.addProjectMember(projectId, userQuery.trim(), role);
      setSuccessMsg(`Successfully shared project with ${userQuery.trim()}!`);
      setUserQuery("");
      await loadMembers();
    } catch (err: any) {
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    ...
    setSuccessMsg(null);
    try {
      await api.updateProjectMember(projectId, userId, newRole);
  ...
  const handleRemoveMember = async (userId: string, username: string) => {
    if (!confirm(`Are you sure you want to remove ${username}'s access to this project?`)) return;
    setError(null);
    setSuccessMsg(null);
    try {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
        <div className="flex items-center justify-between pb-4 border-b border-slate-800">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-base font-bold text-white">Share Project & Permissions</h3>
          </div>
          <button
            onClick={onClose}
